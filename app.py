...
def readme(base_model, lora_name, instance_prompt, sample_prompts):
    ...
    samples_dir = resolve_path_without_quotes(f"/root/autodl-fs/outputs/{output_name}/sample")
    try:
...
def run_captioning(images, concept_sentence, *captions):
    ...
    processor = AutoProcessor.from_pretrained("/root/autodl-tmp/models/Florence2", trust_remote_code=True)
...
def download(base_model):
    ...
    if base_model == "flux-dev" or base_model == "flux-schnell":
        unet_folder = "/root/autodl-tmp/models/unet"
    else:
        unet_folder = f"/root/autodl-tmp/models/unet/{repo}"
    ...
    # download vae
    vae_folder = "/root/autodl-tmp/models/vae"
    ...
    # download clip
    clip_folder = "/root/autodl-tmp/models/clip/"
    clip_l_path = "/root/autodl-tmp/models/clip/clip_l.safetensors"
...
#        --max_grad_norm 0.0 {line_break}"""
  ...
  --max_grad_norm 0.0 {line_break}"""
    ...
    if base_model == "flux-dev" or base_model == "flux-schnell":
        model_folder = "/root/autodl-tmp/models/unet"
    else:
        model_folder = f"/root/autodl-tmp/models/unet/{repo}"
    model_path = os.path.join(model_folder, model_file)
    pretrained_model_path = resolve_path(model_path)

    clip_path = resolve_path("/root/autodl-tmp/models/clip/clip_l.safetensors")
    t5_path = resolve_path("/root/autodl-tmp/models/clip/t5xxl_fp16.safetensors")
    ae_path = resolve_path("/root/autodl-tmp/models/vae/ae.sft")
...
def get_datasets():
    ...
    try:
        ...
        folders.sort(key=lambda folder: os.path.getctime(os.path.join(datasets_path, folder)), reverse=True)
        return folders
    except Exception as e:
...
def get_dataset_info(dataset_name):
    ...
    try:
        ...
        print(f"Error getting dataset info for {dataset_name}: {e}")
        return {"image_count": 0, "files": []}
...
with gr.Blocks(elem_id="app", theme=theme, css=css, fill_width=True) as demo:
    ...
    load_dataset_btn.click(
        ...
        outputs=[images] + output_components
    )
    demo.load(fn=refresh_datasets_list, outputs=[datasets_dropdown])
...
if __name__ == "__main__":
    cwd = os.path.dirname(os.path.abspath(__file__))
    demo.launch(debug=True, show_error=True, allowed_paths=[cwd], server_port=6006)
